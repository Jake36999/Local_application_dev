...
def extract_text_from_image(image_path_or_object) -> str:
    ...
    try:
        ...
        if "tesseract is not installed" in str(e).lower() or "not in your path" in str(e).lower():
             logger.error("Tesseract not found! Please install it and check the path in tools/common/ocr_service.py")
        else:
...
def convert_page_to_image(pdf_path, page_number):
    ...
    try:
        ...
        if "poppler" in str(e).lower():
            logger.error(f"Poppler not found. Please update POPPLER_PATH in tools/common/ocr_service.py. Error: {e}")
        else:
