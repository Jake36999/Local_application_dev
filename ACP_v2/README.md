...
# AUTMATED CODE PLATFORM

## ACP_v2 Orchestrator Upgrade (2026)

### Key Changes

- The orchestrator is now an active watchdog loop, monitoring the staging/incoming directory and dispatching workflows automatically.
- Real workflow functions are registered for ingest, cut_analysis, and governance, replacing previous mock methods.
- A heartbeat status file (`orchestrator.status.json`) is written after each scan interval for system health and API checks.

### Directory Structure

- `staging/incoming/` — Files to be processed
- `staging/processed/` — Successfully processed files
- `staging/failed/` — Files that failed processing

### Next Steps

- Add API endpoint to expose orchestrator status
- Continue to register additional workflows as needed

---
This README was updated as part of the 2026 orchestrator modernization.
